    ...
    while step < training_steps:
        ...
        for i, batch in enumerate(tqdm(train_dial_dataloader)):
            ...
            if opt.use_gt_dbs is False:
                ...
                retriever_context_embeddings = retriver_ent_embs + retriever_context_embeddings
                retriever_context_embeddings, retriever_infer_ent_outs = refer_model(retriever_context_output, seq_lens, ent_mark.long().cuda(),
                ...
                if opt.use_retriever_for_gt is False:
                    ...
                    retriever_rest_dbs_index = torch.cat((retriever_top_k_dbs_index[-1].unsqueeze(0), retriever_top_k_dbs_index[:-1]), dim=0)
                else:
                    # retriever model get top-k db index
                    # 应该是在这里找相关性和db
                    retriver_output = retriever_model(input_ids=retriever_context_input_ids.long().cuda(),
                    ...
                    retriver_ent_embs = retriever_model(input_ids=retriever_context_input_ids.long().cuda(),
                                                        ...
                                                        sent_emb=True).pooler_output  # have grad
                    retriever_context_embeddings = retriver_ent_embs + retriever_context_embeddings
                    retriever_context_embeddings, retriever_infer_ent_outs = refer_model(retriever_context_output,
                                                                                         seq_lens,
                                                                                         ent_mark.long().cuda(),
                                                                                         retriever_context_embeddings)
            ...
            if opt.use_gt_dbs is False:
                ...
                retriever_top_k_dbs_scores = torch.einsum("bad,bkd->bak", retriever_infer_ent_outs.unsqueeze(1),
                ...
                retriever_rest_dbs_scores = torch.einsum("bad,bkd->bak", retriever_infer_ent_outs.unsqueeze(1),
            ...
            if ranker_scores is not None:
                train_loss = train_loss - 0.5 * ranker_model.BinaryCrossEntropy(ranker_scores, rest_ranker_scores)

            if retriever_loss is not None:
                train_loss = train_loss + opt.generator_distill_retriever_loss_alpha * retriever_loss
                # train_loss = train_loss - 0.5 * rest_retriever_loss
            ...
            if (step - 1) % opt.eval_freq == 0 and step > opt.start_eval_step:
                ...
                dev_score, dev_metric = evaluate(generator_model, retriever_model, ranker_model, eval_dial_dataset,
                                                 ...
                                                 generator_db_collator, refer_model, retriever_all_dbs_ids,
                                                 retriever_all_dbs_mask, retriever_all_dbs_token_type)
...
def evaluate(generator_model, retriever_model, ranker_model, eval_dial_dataset, dial_collator, generator_tokenizer, opt,
             ...
             retriever_all_dbs_ids, retriever_all_dbs_mask, retriever_all_dbs_token_type):
        ...
        for i, batch in enumerate(tqdm(eval_dial_dataloader)):
            ...
            if opt.use_gt_dbs is False:
                ...
                retriver_ent_embs = retriever_model(input_ids=retriever_context_input_ids.long().cuda(),
                                                    ...
                                                    sent_emb=True).pooler_output  # have grad
                retriever_context_embeddings = retriver_ent_embs + retriever_context_embeddings
                retriever_context_embeddings, retriever_infer_ent_outs = refer_model(retriever_context_output, seq_lens,
                                                                                     ent_mark.long().cuda(),
                                                                                     retriever_context_embeddings)
